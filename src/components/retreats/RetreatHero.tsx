      ...
      <Container className="relative z-10" size="lg">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <h1 className="font-heading font-bold text-5xl sm:text-6xl lg:text-7xl xl:text-8xl text-cream-50 mb-8 leading-tight tracking-wide mt-8">
            {isWorkshop ? (
              <>
                <span className="block">{retreat.title.toUpperCase()}</span>
              </>
            ) : (
              <>
          ...
          <div className="space-y-4 mb-10">
            ...
            {retreat.venue && (
              <p className="font-body text-lg sm:text-xl text-cream-200 font-medium">
                {retreat.venue}
              </p>
            )}
            {retreat.venueAddress && (
              <p className="font-body text-base sm:text-lg text-cream-300">
                {retreat.venueAddress}
              </p>
            )}
            ...
            {retreat.price && (
              <p className="font-body text-2xl sm:text-3xl text-cream-50 font-bold">
                {retreat.price}
              </p>
            )}
          </div>
          ...
          <div className="inline-flex items-center px-6 py-3 bg-white/10 backdrop-blur-sm rounded-full border border-cream-200/20 text-cream-100 mb-20">
            ...
            <span className="font-body text-sm font-medium">
              {retreat.gearNote || 'Professional equipment kit included'}
            </span>
          </div>
        </div>
      </Container>
